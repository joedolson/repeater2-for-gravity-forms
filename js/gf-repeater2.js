	...
	if (childKey) {
		...
		if (childRequired) {
			...
			if (!childLabel.has('.gfield_required').length) {
				childLabel.append("<span class=\"gfield_required\">Required</span>");
			}
			...
			if (gfRepeater_submitted && checkValidation) {
				if (failedValidation) {
					...
					if (!repeater2ChildElement.has('.validation_message').length) {
						// A11y: will need to get related field to attach via aria-describedby.
...
function gfRepeater_repeatRepeater(formId, repeater2Id) {
	...
	jQuery.each(repeater2['children'], function (key, value) {
		...
		if ( firstEl ) {
			clonedElement.find( 'input' ).trigger( 'focus' );
			firstEl = false;
		}
		...
		els.push( clonedElement );
	});

	els.forEach( ( el ) => {
		gfRepeater_updateLabel( el, repeater2['data']['repeatCount'] );
	});
...
/*
    gfRepeater_updateLabel(inputElement)
        Update the label to increment a count, giving each field a unique label.

        inputElement	The input element.
        count			The repetition number.
*/
function gfRepeater_updateLabel(inputContainer, count) {
	let labels = inputContainer.find( 'label' );
	count += 1;

	if ( labels ) {
		labels.each( function() {
			label = jQuery( this );
			let newEl = jQuery( '<span>', {
				'id' : label.attr( 'for' ) + '_desc',
				'class' : 'label-repeater-item'
			} );
			newEl.text( 'Item ' + count );
			let input = jQuery( '[id="' + label.attr( 'for' ) + '"]' );
			input.attr( 'aria-describedby', label.attr( 'for' ) + '_desc' );
			label.after( newEl );
		});
	}
}
